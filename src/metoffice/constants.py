...
try:
	...
	ADDON_BANNER_PATH = str(home + '/.kodi/addons/%s/resources/banner.png' % ADDON.getAddonInfo('id'))
try:
	ADDON_DATA_PATH = xbmc.translatePath('special://profile/addon_data/%s/' % ADDON.getAddonInfo('id'))
except:
	ADDON_DATA_PATH = str(home + '/.kodi/userdata/addon_data/%s/' % ADDON.getAddonInfo('id'))


try: TEMPERATUREUNITS = unicode(xbmc.getRegion('tempunit'),encoding='utf-8')
except: TEMPERATUREUNITS = str(xbmc.getRegion('tempunit'))
