...
try:
    ...
    from metoffice import utilities, properties, urlcache, astronomy
    ...
    from metoffice.constants import WINDOW, ADDON, API_KEY, CURRENT_VIEW, ADDON_DATA_PATH, ADDON_BANNER_PATH,\
        ISSUEDAT_FORMAT, DATAPOINT_DATETIME_FORMAT,\
        SHORT_DAY_FORMAT, SHORT_DATE_FORMAT, TIME_FORMAT, DATAPOINT_DATE_FORMAT,\
        WEATHER_CODES, WINDOW, DAILY_LOCATION_FORECAST_URL,\
        API_KEY, ADDON_DATA_PATH, THREEHOURLY_LOCATION_FORECAST_URL,\
        TEXT_FORECAST_URL, HOURLY_LOCATION_OBSERVATION_URL,\
...
#    try:
#            utilities.log(str(round(float(latest_obs.get('F', 'n/a')))).split('.')[0]+'========weather')
#    except:
#            utilities.log(str(latest_obs.get('F')).split('.')[0]+'========weather')
        ...
        try:
            WINDOW.setProperty('Current.FeelsLike', feels_like)#@UndefinedVariable
        except:
            WINDOW.setProperty('Current.FeelsLike', 'n/a')#@UndefinedVariable
        #if we get Wind, then convert it to kmph.
        WINDOW.setProperty('Current.Wind', utilities.mph_to_kmph(latest_obs, 'S'))#@UndefinedVariable
        ...
        try:
            ...
            try:
                ...
                WINDOW.setProperty('Current.DewPoint', 'n/a')#@UndefinedVariable
        try:
            WINDOW.setProperty('Current.Humidity', str(humidity))#@UndefinedVariable
        except:
            try: 
                WINDOW.setProperty('Current.Humidity', str(round(float(latest_obs.get('H', 'n/a')))).split('.')[0])#@UndefinedVariable
            except: 
                WINDOW.setProperty('Current.Humidity', 'n/a')#@UndefinedVariable
            ...
            for rep in period['Rep']:
                ...
                if rep.get('$') == 'Day':
                    ...
                    if 'estuary' in SKIN_DIR:
                        ...
                        WINDOW.setProperty('Day%d.OutlookIcon' % p, '%s' % WEATHER_CODES.get(weather_type, 'na')[0])#@UndefinedVariable
                    ...
                    if 'estuary' in SKIN_DIR:
                        ...
                        WINDOW.setProperty('Daily.%d.FanartCode' % (p+1), "%s.png" % WEATHER_CODES.get(weather_type, 'na')[0])#@UndefinedVariable
                    else:
                        WINDOW.setProperty('Daily.%d.OutlookIcon' % (p+1), '%s' % WEATHER_CODES.get(weather_type, 'na')[0])#@UndefinedVariable
                        WINDOW.setProperty('Daily.%d.FanartCode' % (p+1), '%s' % WEATHER_CODES.get(weather_type, 'na')[0])#@UndefinedVariable

                    WINDOW.setProperty('Daily.%d.WindSpeed' % (p+1),  rep.get('S', 'na')+ ' mph')#@UndefinedVariable
                    WINDOW.setProperty('Daily.%d.WindDirection' % (p+1), rep.get('D', 'na'))#@UndefinedVariable
        ...
        for period in dv['Location']['Period']:
            for rep in period['Rep']:
                ...
                if 'estuary' in SKIN_DIR:
                    ...
                    WINDOW.setProperty('Hourly.%d.FanartCode' % count, '%s.png' % WEATHER_CODES.get(weather_type, 'na')[0])#@UndefinedVariable
                else:
                    WINDOW.setProperty('Hourly.%d.OutlookIcon' % count, '%s' % WEATHER_CODES.get(weather_type, 'na')[0])#@UndefinedVariable
                    WINDOW.setProperty('Hourly.%d.FanartCode' % count, '%s' % WEATHER_CODES.get(weather_type, 'na')[0])#@UndefinedVariable
                ...
                import xbmc
                feels_like = utilities.rownd(utilities.localised_temperature(rep.get('F', 'na')))
                from datetime import datetime
                my_date = datetime.strftime(datetime.now(), '%d %b')
                ...
                if my_date == curr_date and my_time > curr_time:
                    ...
                    feels_like1 = utilities.rownd(utilities.localised_temperature(rep.get('F', 'na')))
                    T = float(WINDOW.getProperty('Current.Temperature'))
                    RH = float(humidity)
                    import math
                    rh_pc = RH/100
                    e = math.e
                    dew_point = format(243.04*(math.log(rh_pc , e)+((17.625*T)/(243.04+T)))/(17.625-math.log(rh_pc , e)-((17.625*T)/(243.04+T))),".2f")
                    curr_time2 = curr_time
                    curr_date2 = curr_date

                import math
                v = float(rep.get('S', 'n/a'))*1.60934  #mph => kph
                t = float(rep.get('T', 'na')) # degrees celcius
                WINDOW.setProperty('Hourly.%d.WindChill' % count, str(round(13.12 + 0.6215*t -  11.37*math.pow(v, 0.16) + 0.3965*t*math.pow(v, 0.16),2)))
                WINDOW.setProperty('Hourly.%d.Humidity' % count, str(humidity))
                WINDOW.setProperty('Hourly.%d.ChancePrecipitation' % count, 'Precipitation: ' + rep.get('Pp', 'na') + '%')
                WINDOW.setProperty('Hourly.%d.FeelsLikeTemp' % count,  rep.get('F', 'na')+TEMPERATUREUNITS)
                WINDOW.setProperty('Hourly.%d.FeelsLikeTempIcon' % count, rep.get('F', 'na'))
                WINDOW.setProperty('Hourly.%d.FeelsLike' % count, rep.get('F', 'na')+TEMPERATUREUNITS)
                WINDOW.setProperty('Hourly.%d.FeelsLikeIcon' % count, rep.get('F', 'na'))
...
#    utilities.log('Today.Sunrise = ' + str(sun.sunrise().strftime(TIME_FORMAT)))
#    utilities.log('Today.Sunrise = ' + str(sun.sunset().strftime(TIME_FORMAT)))
#    try:
#        sun = astronomy.Sun(lat=float(LATITUDE), lng=float(LONGITUDE))
#    except:
#    sun = astronomy.Sun(lat=float(55.9533), lng=float(3.1883))
#    utilities.log('Today.Sunrise', sun.sunrise().strftime(TIME_FORMAT))
